...
try:
    ...
    BOT_TOKEN = config("BOT_TOKEN", "8049166513:AAGyVAwwWcjGzxRmVnorfOjDyxkWy23zn00")
    DEV = 7970350353
    OWNER = config("OWNER", "7970350353")
    FFMPEG = config("FFMPEG", default='ffmpeg -i "{}" -preset ultrafast -c:v libx265 -crf 30 -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s? "{}"',)
